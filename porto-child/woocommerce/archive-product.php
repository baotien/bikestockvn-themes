...
?>
	...
	?>
		...
		if( have_rows('link_and_title_menu_child','product_cat_'.$id_term.'') ):
			echo '<ul class="list-menu-child-ct product_cat_'.$id_term.'">';
			// loop through the rows of data
			while ( have_rows('link_and_title_menu_child','product_cat_'.$id_term.'') ) : the_row();?>
				<li>
				<?php
				//var_dump(get_sub_field('link_menu','product_cat_376'));
				//$id_link_menu = get_sub_field('link_menu','product_cat_'.$id_term.'');
				//$termss = get_field('link_menu','product_cat');
				//var_dump($termss);
				//foreach( $termss as $terms ):
					//var_dump($termss);
				//endforeach;
				//echo $id_link_menu;
				//var_dump($id_link_menu);
				//var_dump($id_link_menu->slug);
				?>
					<a href="<?php the_sub_field('link_menu');?>">
						<?php echo get_sub_field('title_menu');?>
					</a>
				</li>
