...
<div class="product-images images">
    ...
    if ( has_post_thumbnail() ) {
		...
		$img_title  	= $product->get_sku();

		$post_terms 	= wp_get_post_terms( get_the_ID() , 'product_cat', array("fields" => "all"));
		$post_term_cr 	= '';
		if( count( $post_terms ) > 0 ){
			foreach( $post_terms as $post_term ){
				if( strpos($post_term->slug, 'san-pham-pho-bien') !== false ){
					$post_term_cr = $post_term;
					break;
				}
			}
		}
		...
		if( $post_term_cr ){
			$folder_name 	= $post_term_cr->slug;
			
			$parent_term 	= get_term( $post_term_cr->parent, 'product_cat' );
			...
			if( file_exists( $file_path ) ){
				$image_link 			= home_url().'/san_pham_pho_bien/'.$folder_name.'/'.$img_title.'.jpg';
			}
		}
