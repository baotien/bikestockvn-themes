...
function my_post_thumbnail_fallback($html, $post_id, $post_thumbnail_id, $size, $attr) {
    if (get_post_type($post_id) == 'product') {
        $is_sp_pho_bien = false;
        global $wpdb;
        $sql = "SELECT * FROM `san_pham_pho_bien` WHERE `post_id` = '" . $post_id . "'";
        $result = $wpdb->get_row($sql);
        if ($result) {
            $term_id = $result->term_id;
            $img_title = $result->sku;
            $post_terms = wp_get_post_terms($post_id, 'product_cat', array("fields" => "all"));
            $post_term = end($post_terms);
            $folder_name = $post_term->slug;
            $parent_term = get_term($post_term->parent, 'product_cat');
            $parent_folder_name = $parent_term->slug;
            $parent_folder_name = str_replace('san-pham-pho-bien-', '', $parent_folder_name);
            $folder_name = $parent_folder_name . '/' . str_replace('-san-pham-pho-bien-' . $parent_folder_name, '', $folder_name);
            $file_path = get_home_path() . 'san_pham_pho_bien/' . $folder_name . '/' . $img_title . '.jpg';
            if (file_exists($file_path)) {
                $image = home_url() . '/san_pham_pho_bien/' . $folder_name . '/' . $img_title . '.jpg';
                $html = '<img src="' . $image . '">';
                return $html;
            }
        }
...
function advance_searh_woo() {
    ...
    if (!is_shop() && $layout == 'grid') {
        ...
        <div class="row">
            <div class="col-lg-8"><button id="back_button" class="btn btn-primary showlogin pull-left">Back</button></div>
            <div class="col-lg-4">
        ...
        <script>
            jQuery(document).ready(function () {
                ...
                if (brand_id && model_id) {
                    jQuery('#brand option[value="<?php echo $brand_name; ?>"]').prop('selected', true);
                ...
                function get_product_item(pro_id) {
                    ...
                    jQuery.ajax({
                        ...
                        dataType: "html"
                            ...
                            .done(function (respon) {
                                jQuery('.archive-products .products').html(respon);
                                jQuery('.archive-products .products').addClass('ajax_complete');
                                remove_loading_ajax();
