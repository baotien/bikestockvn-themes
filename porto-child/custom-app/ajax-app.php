...
function ajax_get_product_item(){
	...
	$is_product_childs 	= get_post_meta($pro_id,'phu_tung_ids',true);
	$is_product_childs  = explode(',',$is_product_childs);
	ob_start();
	?>
	<div class="row">
		<?php if( $is_image_hotspot ): ?>
			...
			<?php if( $id_hotspot ): ?>
				...
				<div class="col-md-6 summary entry-summary custom-height-scroll">
					...
					<table id="hotspot_items" style="margin-bottom:0px;" class="table table data_head aaaaaaa">
						<thead style="">
								<tr style="color: #333;">
								...
								foreach ($id_hotspots_points as $id_hotspots_point) {
								   ...
								   ?>
									<tr class="quickview-add-to-tmp-cart menu-item hotspot-item hotspot-item-id-<?php echo $id_hotspots_point['id'];?>" data-id="<?php echo $id_hotspots_point['id'];?>" data-object="<?php echo $id_hotspots_point['object_id'];?>">
										...
										<td width="20%"><span class=""><?php echo $price;?></span></td>
									</tr>
		...
		<?php elseif( count( $is_product_childs ) > 0 ): ?>
			<div class="col-md-6 summary-before">
				<?php 
					$post_terms 	= wp_get_post_terms($pro_id, 'product_cat', array("fields" => "all"));
					$post_term	 	= end($post_terms);
					$folder_name 	= $post_term->slug;

					$feat_image_url = wp_get_attachment_url( get_post_thumbnail_id($pro_id) );	
					$img_title = get_post($pro_id);
					$img_title = $img_title->post_name;
					$img_title = strtoupper($img_title);
					$img_title = str_replace("’","'",$img_title);
					if( file_exists( '/home/cp403490/public_html/bikestockvn.com/phu_tung_images/'.$folder_name.'/'.$img_title.'.jpg' ) ){
						$feat_image_url 			= home_url().'/phu_tung_images/'.$folder_name.'/'.$img_title.'.jpg';
					}elseif( file_exists( '/home/cp403490/public_html/bikestockvn.com/phu_tung_images/'.$folder_name.'/'.$img_title.'.JPG' ) ){
						$feat_image_url 			= home_url().'/phu_tung_images/'.$folder_name.'/'.$img_title.'.JPG';
					}else{
						$feat_image_url 			= home_url().'/phu_tung_images/'.$folder_name.'/'.$img_title.'.jpg';
					}
				...
				<table id="hotspot_items" style="margin-bottom:0px;" class="table table data_head">
					<thead style="">
							<tr style="color: #333;">
								...
								<th width="25%" >Part No</th>
								<th width="35%" >Name</th>
								<th width="10%" class="sug-product">Sug.Q</th>
								<th  width="20%" style="border-top-right-radius: 5px;">Price</th>
							</tr>
					</thead>
					<tbody class="">
						<?php
							...
							foreach ($is_product_childs as $is_product_child) {
								$p_data       = get_post($is_product_child); 
								$_product     = wc_get_product( $is_product_child );
								if( $_product->price ){
									$price          = $_product->get_price_html();
								}else{
									$price          = '<span class="woocommerce-Price-amount amount">Liên Hệ</span>';
								...
								$p_data_title = $p_data->post_title;
								$sku 		= get_post_meta($is_product_child,'_sku',true);
								$stt 		= get_post_meta($is_product_child,'stt',true);
								$sg_qty 	= get_post_meta($is_product_child,'so_luong',true);
								$i++;
							  
							   ?>
								<tr class="quickview-add-to-tmp-cart menu-item hotspot-item hotspot-item-id-<?php echo $is_product_child;?>" data-id="<?php echo $is_product_child;?>">
									<td width="10%"><span class="pro-no"><?php echo $stt;?></span></td>
									<td width="25%"><span class="pro-sku"><?php echo $sku;?></span></td>
									<td width="35%"><span class="pro-title"><?php echo $p_data_title;?></span></td>
									<td width="10%" class="sug-product"><span class="pro-qty"><?php echo $sg_qty;?></span></td>
									<td width="20%"><span class=""><?php echo $price;?></span></td>
									<td><span class="pro-price" style="display: none;"><?php echo $_product->price;?></span></td>
								</tr>
			   ...
			   <tfoot>
					<tr>
					  <td colspan="5" class="btn-cart-action hide">
						...
						<a class="btn btn-default pull-right btn-cart-action-link hide" href="<?php echo home_url();?>/cart"> Xem Lại Giỏ Hàng</a>
					  </td>
					</tr>
				  </tfoot>
			</table>
	...
	jQuery( document ).ready( function(){
		...
		jQuery('body').on('click','.data_add_to_cart .close_button',function(){
			...
			if( jQuery('.data_add_to_cart').length == 0 ){
                ...
                function format_money(n) {
                    var str = n.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,") + " đ";
		...
		jQuery('body').on('click','.quickview-add-to-tmp-cart',function(){
			...
			var pro_price       = jQuery(this).find('ins .woocommerce-Price-amount').html();
                        var pro_price_value = jQuery(this).find('.pro-price').text();
			...
			xhtml+= '<tr class="data_add_to_cart to-tmp-cart-id-'+pro_id+'" data-id="'+pro_id+'">';
					xhtml += '<td width="10%">'+pro_no+'</td>';
					xhtml += '<td width="20%">'+pro_sku+'</td>';
					xhtml += '<td width="35%" class="product_name">'+pro_title+'</td>';
					xhtml += '<td width="5%" class="qty">';
					...
					xhtml += '</td>';
					xhtml += '<td width="20%" class="price">'+format_money(pro_price_value*pro_qty)+'</td>';
					xhtml += '<td style="display: none;">'+pro_price_value+'</td>';
					xhtml += ' <td width="5%" style="text-align: center;"><i class="fa fa-trash-o close_button" data-id="'+pro_id+'"></i></td>';
			xhtml+= '</tr>';
			...
			jQuery('.btn-cart-action').removeClass('hide');
                        jQuery(".to-tmp-cart-id-"+pro_id).find(".qty_value").on("change paste keyup", function() {
                            var pro_qty = jQuery(this).val();
                            jQuery(".to-tmp-cart-id-"+pro_id).find('td.price').html(format_money(pro_price_value*pro_qty));
                        });
		});
...
function ajax_get_products(){
	...
	if( count( $products ) > 0 ){
		...
		foreach( $products as $product ){
			...
			$img_title 		= $product->post_name;
			$img_title 		= strtoupper($img_title);
			$post_terms 	= wp_get_post_terms($product->ID, 'product_cat', array("fields" => "all"));
			$post_term	 	= end($post_terms);
			$folder_name 	= $post_term->slug;
			
			if( file_exists( '/home/cp403490/public_html/bikestockvn.com/phu_tung_images/'.$folder_name.'/'.$img_title.'.jpg' ) ){
				$image 			= home_url().'/phu_tung_images/'.$folder_name.'/'.$img_title.'.jpg';
			}elseif( file_exists( '/home/cp403490/public_html/bikestockvn.com/phu_tung_images/'.$folder_name.'/'.$img_title.'.JPG' ) ){
				$image 			= home_url().'/phu_tung_images/'.$folder_name.'/'.$img_title.'.JPG';
			}else{
				$image = 'https://placeholdit.imgix.net/~text?txtsize=33&txt='.$img_title.'&w=337&h=215';
			}
			ob_start();
			?>
				<li class="show-links-onimage product">
